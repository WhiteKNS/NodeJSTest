Feature: Delete User by ID Clients should be able to send a request to our API in order to delete a user.
Background: Create two Users and logs in with the first user's account
    Given 2 new user is created with random password and email
...
Scenario: Delete Self with Token with Wrong Signature The user is trying to delete its own account, the token contains the correct payload, but the signature is wrong.
    When the client creates a DELETE request to /users/:userId
...
Scenario: Delete Different User A user can only delete him/herself. When trying to delete another user, it should return with 403 Forbidden.
